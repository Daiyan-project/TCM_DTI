...
def main():
    ...
    for i in trange(0,len(input_scaffolds)):
        scaffold1=[]
        scaffold1.append(input_scaffolds[i])
        results_df = sample_scaffolds.run(scaffold1)
        if args.output_format == "parquet":
            results_df.write.parquet(args.output_path)
        else:
            results_df.toPandas().to_csv(args.output_path, mode='a' if os.path.exists(args.output_path) else 'w', header=False, index=False)
